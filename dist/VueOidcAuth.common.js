...
function createOidcAuth(_authName, defaultSignInType, _appUrl, oidcConfig) {
  ...
  mgr.events.addAccessTokenExpired(function () {
    external_oidc_["Log"].debug("".concat(_authName, " auth token expired"));

    if (auth.isAuthenticated) {
      mgr.signinSilent().then(function () {
        external_oidc_["Log"].debug("".concat(_authName, " auth silent signin after token expiration"));
      }).catch(function () {
        external_oidc_["Log"].debug("".concat(_authName, " auth silent signin error after token expiration"));
        signInIfNecessary();
      });
    }
  });
  ...
  function signInIfNecessary() {
    if (auth.myRouter) {
      ...
      if (current && current.meta.authName === _authName) {
        external_oidc_["Log"].debug("".concat(_authName, " auth page re-signin"));
